...
const TradeVolumePage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                console.error('Error details:', err); // 자세한 에러 확인용
    ...
    if (isLoading) return <div className="flex-1 flex justify-center items-center">로딩 중...</div>;
    ...
    return (
        <div className="px-4">
            ...
            <h1 className="text-2xl font-bold mb-4">거래량 순위</h1>
                ...
                <table className="w-full">
                    <thead>
                        <tr>
                            ...
                            <th className="p-3 bg-blue-500 text-white border">전일거래량</th>
                        </tr>
                    </thead>
                    <tbody>
                        {volumeData.map((item, index) => (
                            <tr key={index} className="hover:bg-gray-50">
                                ...
                                <td className="p-3 border text-right">
                                    {Number(item.prdy_vol).toLocaleString()}
                                </td>
                            </tr>
                        ))}
                    </tbody>
